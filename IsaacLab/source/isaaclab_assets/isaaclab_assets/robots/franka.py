...
_LOCAL_FRANKA_DIR = "/home/krz/isaaclab_ws/assets/robots/franka"
_LOCAL_PANDA_USD = os.path.join(_LOCAL_FRANKA_DIR, "panda_instanceable.usd")
_LOCAL_PROPS_DIR = os.path.join(_LOCAL_FRANKA_DIR, "Props")
_LOCAL_MATERIALS_DIR = os.path.join(_LOCAL_FRANKA_DIR, "Materials")
# Use local override only when Props/ and Materials/ exist (avoids Materials.usd warnings).
_PANDA_USD_PATH = (
    _LOCAL_PANDA_USD
    if (
        os.path.isfile(_LOCAL_PANDA_USD)
        and os.path.isdir(_LOCAL_PROPS_DIR)
        and os.path.isdir(_LOCAL_MATERIALS_DIR)
    )
    else f"{ISAACLAB_NUCLEUS_DIR}/Robots/FrankaEmika/panda_instanceable.usd"
)
